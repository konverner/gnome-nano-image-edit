...
def _create_text_css_provider(color_rgba: tuple = (255, 255, 255, 255)) -> Gtk.CssProvider:
...
class CanvasWidget(Gtk.DrawingArea):
    ...
    def __init__(self, processor, manager):
        ...
        self._drag_mode = 'none' # 'none', 'select', 'move'
    ...
    def _calculate_image_display_geometry(self):
        ...
        img_w, img_h = self.processor.current_image.get_width(), self.processor.current_image.get_height()
        ...
        self._image_display_rect = (x_offset, y_offset, display_width, display_height, scale)
    ...
    def on_draw(self, area, cr, width, height):
        ...
        # Draw selection rectangle only when creating a new selection
        if (self._drag_mode == 'select' or self._drag_mode == 'text_create') and self.selection_box:
            cr.save()
    ...
    def on_drag_begin(self, gesture, start_x, start_y):
        ...
        if handle and self.processor.current_image:
            ...
            self._resize_start_size = (self.processor.current_image.get_width(), self.processor.current_image.get_height())
            self._resize_anchor = self._anchor_for_handle(handle)
            self._canvas_resize_in_progress = True
            gesture.set_state(Gtk.EventSequenceState.CLAIMED)
            return

        # Skip drag operations for text tool - clicks are handled by on_canvas_pressed
        ...
        if current_tool == 'select':
            if self.processor._floating_selection_data:
                ...
                if w_coords and w_coords2 and w_coords[0] <= start_x <= w_coords2[0] and w_coords[1] <= start_y <= w_coords2[1]:
                    self._drag_mode = 'move'
                    self._drag_start_offset = (start_x - w_coords[0], start_y - w_coords[1])
                else:
        ...
        if self._drag_mode == 'select' and self.selection_box:
            ...
            if scaled_selection and (scaled_selection[2] > scaled_selection[0]) and (scaled_selection[3] > scaled_selection[1]):
                if self.manager.current_tool == 'select':
                    ...
                    # For crop, we just set the selection in the processor
    ...
    def _hit_test_resize_handle(self, x, y):
        ...
        if on_top:
            if on_left: return 'top-left'
            if on_right: return 'top-right'
            return 'top'
        if on_bottom:
            if on_left: return 'bottom-left'
            if on_right: return 'bottom-right'
            return 'bottom'
